      ...
      async start(controller) {
        ...
        try {
          ...
          for await (const event of eventStream) {
            ...
            console.log(`[Stock API] Event - Next: ${next}, Messages: ${messages.length}`);
            ...
            // Check if final response - the finalResponse node sets next to END which becomes "__end__"
            // Also check if message doesn't start with [Routing to to identify actual content
            const isFinalResponse = next === "__end__" || 
                                   (next === "finalResponse" && messageContent && !messageContent.includes("[Routing to"));
